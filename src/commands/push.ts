...
import { importNearbyGamePngs } from '../util/gameScan.js';
 ...
 * Used by `magicpixel push` and by `sync` (so connect/watch imports game
 * sprites without a second command).
 */
...
export async function runPushWith(
  ...
  const scan = await importNearbyGamePngs(config.outDir, kind);
  const imported = scan.copied;
  ...
  if (sendable.length === 0) {
    ...
    return { ...empty, imported, unchanged: skipped };
  }
      ...
      console.log(
        kleur.dim(
          `  ${counts.created} new sprite${counts.created === 1 ? '' : 's'} now live in your MagicPixel library ` +
            `(folders mirrored from ${relative(process.cwd(), outRoot)}). Refresh the library to see them.`,
        ),
      );
    }
  }
